...
### Task 5: Skeleton / loading-feedback system (homepage + trending + intro fallback)

Add visible "something is happening" feedback on the only storefront page that currently shows nothing while data loads. Admin pages, all admin buttons (`Button loading=`), checkout, and auth already have feedback — verified during scoping, no changes there.

**Files:**
- Modify: `frontend/src/components/product/TrendingSection.tsx` (add `loading` prop)
- Modify: `frontend/src/app/page.tsx` (featured skeleton grid + IntroScene fallback)
- Test: `frontend/src/components/product/__tests__/TrendingSection.test.tsx` (new)

**Interfaces:**
- `interface TrendingSectionProps { products: Product[]; loading?: boolean }` — when `loading` is true (and `products` empty), render 4 shimmer skeleton cards instead of an empty scroll row.
- `page.tsx`: add `featuredLoading` state (init `true`, `false` after fetch resolves/rejects). Featured Products grid renders `<SkeletonCard count={4} />` while `featuredLoading`. TrendingSection gets `loading={featuredLoading}`.
- `page.tsx`: replace `IntroScene` dynamic import `loading: () => null` with a branded fallback (centered `Logo` + "Loading immersive experience…" + bouncing dots, using existing `Logo` component + Tailwind animations).

- [ ] **Step 1: Write the failing TrendingSection test**

Create `frontend/src/components/product/__tests__/TrendingSection.test.tsx`:
...
Run from `frontend/`: `npx vitest run src/components/product/__tests__/TrendingSection.test.tsx`

Expected: FAIL — `loading` prop not accepted and no `data-testid="trending-skeleton"` exists.

- [ ] **Step 3: Implement `TrendingSection` loading prop**

Edit `frontend/src/components/product/TrendingSection.tsx`:
- Signature → `function TrendingSection({ products, loading = false }: TrendingSectionProps)` and add `loading?: boolean` to the interface.
- Above the `{products.map(...)}` block (lines 85-87), render when `loading && products.length === 0`:
...
- [ ] **Step 4: Implement homepage skeleton states**

Edit `frontend/src/app/page.tsx`:
- Add `const [featuredLoading, setFeaturedLoading] = useState(true);` next to `featuredProducts` state.
- In the fetch effect: `setFeaturedLoading(false)` in both `try` (after `setFeaturedProducts`) and `catch`.
- Featured Products grid (lines 283-295): when `featuredLoading` render `<SkeletonCard count={4} />` inside the grid container instead of the empty product map. Keep the section heading and "View All" link visible.
- TrendingSection (line 268): pass `loading={featuredLoading}`.
- Replace the `IntroScene` dynamic import block (lines 20-23):
...
Add `import Logo from '@/components/ui/Logo';` to the imports. `Logo` is `{ className, size, withText, name }` with default `name="IMMERSIVE"` — use `withText={false}` since the wordmark is rendered as the separate caption line below.
...
Open `http://localhost:3000` — on slow network (or throttling in DevTools), the intro fallback and homepage featured-skeleton grid appear instead of blank/empty regions. Regression: `/products`, `/admin` still show their existing skeletons.

- [ ] **Step 8: Commit**

```bash
git add frontend/src/components/product/TrendingSection.tsx frontend/src/components/product/__tests__/TrendingSection.test.tsx frontend/src/app/page.tsx
git commit -m "feat(frontend): add skeleton loading feedback to homepage"
```
